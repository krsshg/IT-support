<?php
   include $_SERVER["DOCUMENT_ROOT"] . "/functions/includes/session.php";
?>
<!DOCTYPE html>
<html lang="en">
<head>
</head>
    <?php include $_SERVER["DOCUMENT_ROOT"] . "/functions/start.php";?>
  <body>
    <div class="container">

    <div class="jumbotron">
      <img src="../../img/Elkem corporate logo.png" width="190px" height="90px"/>
      <div align="center">
        <h2> Opprette ekstern VPN bruker hjelper</h2>
          <hr><br>

            <div class = "utlaanButtonsAlignCenter">

                  <button type=button id="linemanager" onclick="btnTglextMail()">Lag mail</button>
                  <br>
                  <div style="display:none" id=extMailData>
                    <div class="tilgjengeligPC">
                      <?php include 'extMailCreate.php'; ?>
                    </div>
                  </div>
                  <br>
          <!--
                  <button type=button id="linemanager" onclick="btnTglLDAPtest()">LDAP testing</button>
          <br>
                  <div style="display:none" id=LDAP>
                  <div class="tilgjengeligPC">
                    <?php //include '../LDAP/ldapTest.php'; ?>
                  </div>
                </div>
          -->

              </div><!--utlaanButtonsAlignCenter-->


    </div>
    </div>
  </div> <!-- /container -->
  <script>
  function btnTglextMail() {
      var x = document.getElementById("extMailData");
      if (x.style.display === "none") {
          x.style.display = "block";
      } else {
          x.style.display = "none";
      }
  }
  </script>
  </body>
</html>
